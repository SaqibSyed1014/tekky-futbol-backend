...
def _valid_payload(**overrides) -> dict:
    ...
    payload = {
        ...
        # Signature image must be ≥100 chars (validated by serializer)
...
class WaiverSubmitTests(BaseAPITestCase):
    ...
    def test_under_18_date_of_birth_returns_400(self):
        player = self.create_player()
        self.authenticate_as(player)

        # Date of birth 10 years ago
        dob = (date.today() - timedelta(days=365 * 10)).isoformat()
        payload = _valid_payload(date_of_birth=dob)
    ...
    def test_missing_one_clause_returns_400(self):
        ...
        clauses["age_18_or_older"] = False          # one clause not checked
