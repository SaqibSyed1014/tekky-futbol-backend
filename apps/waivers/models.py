...
# All 11 must be True for a submission to be accepted.
# NOTE: age_18_or_older was removed — age is now captured via the is_minor flag
# and a separate age-group declaration, allowing minors with guardian sign-off.
REQUIRED_CLAUSES = [
...
class WaiverSignature(models.Model):
    ...
    participant_phone       = models.CharField(max_length=20, blank=True, default="")

    # ── Minor / guardian ──────────────────────────────────────────────────────
    # is_minor=True means the participant is under 18; guardian fields required.
    is_minor              = models.BooleanField(default=False)
    guardian_signature    = models.TextField(blank=True, default="")
    guardian_name_printed = models.CharField(max_length=100, blank=True, default="")
    guardian_email        = models.EmailField(blank=True, default="")
    guardian_phone        = models.CharField(max_length=20, blank=True, default="")
    guardian_type         = models.CharField(max_length=20, blank=True, default="")  # 'parent' | 'legal_guardian'

    # ── 11 initialled clauses ─────────────────────────────────────────────────
    # Stored as {"voluntary_participation": true, ...}
    # All 11 keys must be present and true — enforced at the serializer level.
    clauses_initialed = models.JSONField(default=dict)

    # ── Signature block ───────────────────────────────────────────────────────
    printed_name    = models.CharField(max_length=100, blank=True, default="")
    signature_image = models.TextField(blank=True, default="")   # base64-encoded PNG or typed name
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=["is_minor"], name="waiver_is_minor_idx"),
        ]

    def __str__(self) -> str:
        label = "minor" if self.is_minor else "adult"
        return f"{self.user.email} — waiver v{self.waiver_version} ({label}) signed {self.signed_at:%Y-%m-%d}"
